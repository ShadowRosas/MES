...
tags_to_read2 = ['Frnt5069UDT[{}]'.format(i) for i in range(17)]
tags_to_read3 = ['Rear5069UDT[{}]'.format(i) for i in range(17)]
        ...
        try:
            with LogixDriver(plc_address) as plc:
                for tag in tags:
                    ...
                    # Leer el valor de las etiquetas
                    try:
                        ST_ID = plc.read(f'{tag}.ST_ID').value or ''
                        PassFail = plc.read(f'{tag}.PassFail').value or ''
                        StartTm = plc.read(f'{tag}.StartTm').value or ''
                        EndTm = plc.read(f'{tag}.EndTm').value or ''
                        Sn = plc.read(f'{tag}.Sn').value or ''
                        VarData = plc.read(f'{tag}.VarData').value or ''
                        TestSpec = plc.read(f'{tag}.TestSpec').value or ''
                        Trigger = plc.read(f'{tag}.Trigger').value or ''
                        PartStatus = plc.read(f'{tag}.PartStatus').value or ''
                        ...
                        data_object = {
                            ...
                            **{subtag: str(value) for subtag, value in (data_structure or {}).items()}
                        }
                        
                        plc_ip = plc_address  # Usa la IP proporcionada
                        ...
                        print(f"Error reading tag {tag}: {e}", flush=True)  # Mensaje de depuración
...
thread1 = threading.Thread(target=read_and_send_tags, args=('192.168.50.10', tags_to_read, url,plc_ips))
thread2 = threading.Thread(target=read_and_send_tags, args=('192.168.50.40', tags_to_read2, url,plc_ips))
thread3 = threading.Thread(target=read_and_send_tags, args=('192.168.50.70', tags_to_read3, url,plc_ips))
# Iniciar los hilos
thread1.start()
thread2.start()
thread3.start()
